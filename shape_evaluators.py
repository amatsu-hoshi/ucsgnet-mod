...
class ShapeEvaluator(nn.Module): 
    def __init__(
        ...
        self.num_all_parameters = self.num_parameters + num_dimensions + num_rotation_parameters
        self.shape_params = torch.zeros(1, self.num_of_shapes, num_parameters, dtype=torch.float)
        self.translation = torch.zeros(1, self.num_of_shapes, num_dimensions, dtype=torch.float)
        self.rotation = torch.zeros(1, self.num_of_shapes, num_rotation_parameters, dtype=torch.float)
        self.rotation[..., 0] = 1
        self.shape_params = nn.parameter.Parameter(
            self.shape_params, 
            requires_grad=True) 
        self.translation = nn.parameter.Parameter(
            self.translation, 
            requires_grad=True) 
        self.rotation = nn.parameter.Parameter(
            self.rotation, 
            requires_grad=True) 
...
class CircleSphereEvaluator(ShapeEvaluator):
    def __init__(self, num_of_shapes: int, num_dimensions: int):
        ...
        with torch.no_grad(): 
            nn.init.constant_(self.shape_params, 0.25)
...
class CompundEvaluator(nn.Module):
    ...
    def forward(self, points: torch.Tensor) -> torch.Tensor:
        return torch.cat([part(points) for part in self.parts], dim=1)
...
def create_compound_evaluator(
    number_of_shapes: dict, num_dimensions: int
) -> t.Union[CompundEvaluator, PlanesEvaluator]:
    evaluators = []
    evaluator_type = {
        'sphere': CircleSphereEvaluator, 
        'box': SquareCubeEvaluator
    }
    for shape_type, num_shapes in number_of_shapes.items(): 
        evaluators.append(evaluator_type[shape_type](num_shapes, num_dimensions))
    return CompundEvaluator(evaluators)
